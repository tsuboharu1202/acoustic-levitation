...
class BallPopController_RL:
    ...
    def calculate_reward(self,u,v,vx):
        distance = np.sqrt((u - self.tgtU)**2 + (v - self.tgtV)**2)
        loss = 0
        if (vx > 5/scale) or (vx < -5/scale):
            loss = -10000

        if distance < 1/scale:
            return 25600000 + loss  
        elif distance < 3/scale:
            return 1280000 + loss  
        elif distance < 5/scale:
            return 640000 + loss  
        elif distance < 7/scale:
            return 10000 + loss  
        elif distance < 10/scale:
            return 0 + loss  
        elif distance < 15/scale:
            return -10000 + loss  
        else:
            return -20000000 - distance**4 + loss  
    ...
    def choose_action(self,state):
        if np.random.rand() < epsilon:
            return np.random.randint(len(forces)), np.random.randint(len(angles))
        else:
            idx = np.argmax(self.Q_table[state])
            return idx // len(angles), idx % len(angles)
        

    def discretize_state(self,vx, vy, x, y):
        vx_bin = np.digitize(vx, velocity_bins) - 1
        vy_bin = np.digitize(vy, velocity_bins) - 1
        r = np.sqrt((x - self.tgtU)**2 + (y - self.tgtV)**2)
        theta = np.arctan2(y - self.tgtU, x - self.tgtV)
        # theta の値を -pi から pi の範囲に正規化
        theta = (theta + np.pi) % (2 * np.pi) - np.pi
        r_bin = np.digitize(r, distance_bins) - 1
        theta_bin = np.digitize(theta, theta_bins) - 1
        return vx_bin, vy_bin, r_bin, theta_bin




    def GetOffsetAndAmp_RL(self,u,v,velU,velV,time):
        self.state = self.discretize_state(u,v,velU,velV)
        action = self.choose_action(self.state)
        force = forces[action[0]]
        angle = angles[action[1]]
        amp = force
        ...
        if type(self.buffer) != type(None):
            ...
            if matrix is not None:
                ...
                if detected is not None:
                    ...
                    amp = forces[self.action[0]]
                    angle = angles[self.action[1]]
